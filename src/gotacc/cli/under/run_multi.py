...
def build_interface(machine_cfg: dict, run_cfg: dict):
    """
    Build the evaluation interface for a multi-objective workflow.

    This skeleton expects the config to provide a multi-objective interface class.
    Example:
        machine_cfg["interface_class"] = "gotacc.interfaces.epics_multi.MultiObjEpicsIOC"
    """
    if "interface_class" not in machine_cfg:
        raise KeyError(
            "machine_cfg['interface_class'] is required for multi-objective runner."
        )

    InterfaceClass = import_from_string(machine_cfg["interface_class"])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run GOTAcc multi-objective workflow.")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Dotted config module path, e.g. gotacc.configs.irfel_online",
    )
...
def main():
    ...
    try:
        ...
        ref_point = np.asarray(machine_cfg["ref_point"], dtype=float)
        ...
        if run_cfg.get("set_best", False) and hasattr(objhub, "set_best"):
            objhub.set_best(
                maximize=opt_cfg.get("maximize", False),
                selector=run_cfg.get("best_selector", "sum"),
            )

        print(f"[run_multi] total time = {time.time() - t0:.2f} s")
